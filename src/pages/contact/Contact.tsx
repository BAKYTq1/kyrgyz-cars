...
const contactCardsConfig = [
  {
    ...
    accent: "purple", // Изменено на purple
  },
  {
    step: "2",
    icon: "phone",
...
const reviewsData: Review[] = [
  ...
  },
  {
    ...
    platform: "facebook",
    rating: 5,
    text: "Ogromnie jestem очень доволен. Все по плану.",
    name: "Łukasz Jumel",
    date: "3 месяца назад",
    avatar: teamImg1,
  },
...
const sourceBadgeClasses: Record<ReviewPlatform, string> = {
  google: "bg-purple-500", // Изменено на purple-500
  ...
  return (
    <section className="bg-gray-50 text-gray-950">
      <div className="mx-auto w-[min(1110px,calc(100%-28px))] py-8 md:py-10">
        ...
        {/* Карточки контактов (заменены границы, бэджи и текст кнопок на фиолетовый) */}
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {contactCards.map((card) => (
            <article
              key={card.title}
              className="relative min-h-52 rounded-lg border border-purple-200 bg-white p-5 shadow-sm"
            >
              <span className="absolute -left-2 -top-2 grid h-7 w-7 place-items-center rounded-full bg-purple-600 text-xs font-black text-white">
                {card.step}
              </span>
              <div className="flex items-start justify-between gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-md bg-purple-600 text-white">
              ...
              <button
                className="mt-5 text-sm font-black text-purple-600 hover:text-purple-700 transition"
                type="button"
              >
                {card.action}
              </button>
      ...
      {/* Секция отзывов (активные табы, ховеры и рейтинг стали фиолетовыми) */}
      <div className="border-y border-gray-200 bg-white py-10 md:py-12">
        <div className="mx-auto w-[min(1110px,calc(100%-28px))]">
                ...
                <button
                  ...
                  className={`h-11 shrink-0 rounded-lg border px-4 text-sm font-bold transition ${
                    isActive
                      ...
                      : "border-gray-200 bg-white text-gray-700 hover:border-purple-600 hover:text-purple-600"
                  }`}
                  type="button"
          ...
          <div ref={containerRef} className="relative mt-3">
            ...
            {/* Пагинация (стрелки и точки ховерятся фиолетовым) */}
            {totalPages > 1 && (
              <div className="mt-6 flex items-center justify-center gap-4">
                <button
                  ...
                  className="grid h-9 w-9 place-items-center rounded-full border border-gray-300 bg-white text-gray-600 shadow-sm transition hover:border-purple-500 hover:text-purple-600 disabled:cursor-not-allowed disabled:opacity-30"
                >
      ...
      {/* Адрес и карта (иконка маркера теперь фиолетовая) */}
      <div className="mx-auto w-[min(1110px,calc(100%-28px))] py-10 md:py-12">
        <div className="border-t border-gray-300 pt-8">
          <div className="grid gap-6 md:grid-cols-[1fr_320px] md:items-start">
            ...
            <div className="flex gap-4 rounded-lg bg-white p-4 shadow-sm">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-md bg-purple-600 text-white">
        ...
        <section className="mt-12 overflow-hidden">
          <h2 className="text-2xl font-black tracking-tight md:text-xl">
