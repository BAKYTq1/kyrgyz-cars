...
const fuelMap: Record<string, string> = { '0': '—', '1': 'Бензин', '2': 'Дизель', '3': 'Гибрид', '4': 'Электро' }
...
export default function LotDetailPage() {
  ...
  if (error || !car) return (
    <div className="min-h-screen flex items-center justify-center text-gray-500">Лот не найден</div>
  )

  const auction: 'Copart' | 'IAAI' = car.lot.startsWith('1-') ? 'Copart' : 'IAAI'
  const images = Object.values(car.img_large ?? car.img).filter(Boolean) as string[]
  const buyNow = car.buy_now_price && Number(car.buy_now_price) > 0 ? Number(car.buy_now_price) : null
  ...
  const specs = [
    { label: 'Пробег', value: car.odometer_substr ? `${car.odometer_substr.toLocaleString()} mi` : `${car.odometer.toLocaleString()} mi` },
    ...
    { label: 'Документ', value: car.sale_document_external ?? car.sale_document ?? '—' },
    ...
    <div className="bg-gray-50 min-h-screen py-4 px-4">
      <div className="max-w-[1240px] mx-auto">
        ...
        <div className="flex items-start justify-between flex-wrap gap-2 mb-4">
          <div>
            <h1 className="text-xl font-black text-gray-900">{car.name_long}</h1>
            <div className="flex items-center gap-2 mt-1.5">
              <span className={`text-xs font-bold px-2.5 py-1 rounded-md text-white ${auction === 'Copart' ? 'bg-blue-600' : 'bg-red-500'}`}>
                {auction}
              </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <div className="relative group">
                ...
                <div className="absolute top-3 left-3 flex gap-1.5">
                  {car.has_video && (
                    <span className="text-xs bg-black/60 text-white px-2 py-0.5 rounded-md">▶ Видео</span>
                  )}
                  {car.has_360_view && (
                    <span className="text-xs bg-black/60 text-white px-2 py-0.5 rounded-md">360°</span>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              <h2 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                <FaFileAlt className="text-blue-500" /> Документы и особенности
              </h2>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Номер лота</p>
                  <p className="text-sm font-semibold text-gray-800">{car.lot}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">VIN</p>
                  <p className="text-sm font-semibold text-gray-800 font-mono break-all">{car.vin}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {car.seller_trusted && (
                  <span className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-3 py-1 rounded-full border border-blue-100">
                    <FaShieldAlt /> Надёжный продавец
                  </span>
                )}
                {car.has_video && (
                  <span className="flex items-center gap-1 text-xs bg-green-50 text-green-700 px-3 py-1 rounded-full border border-green-100">
          ...
          <div>
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 sticky top-4 space-y-4">
              ...
              {/* Текущая ставка */}
              <div>
                <p className="text-xs text-gray-400 mb-1">{t('lotCard.currentBid')}</p>
                <p className="text-3xl font-black text-gray-900">
                  {car.final_bid_formatted ?? car.prebid_price}
                </p>
                {car.estimated_min > 0 && (
                  <p className="text-xs text-gray-400 mt-1">
                    Оценка: ${car.estimated_min.toLocaleString()} — ${car.estimated_max.toLocaleString()}
                  </p>
                )}
              </div>
              ...
              {buyNow && (
                <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                  <p className="text-xs text-gray-500 mb-0.5">{t('lotCard.buyNow')}</p>
                  <p className="text-2xl font-black text-blue-600">${buyNow.toLocaleString()}</p>
                </div>
              )}
              ...
              {/* Форма ставки */}
              <div>
                <label className="text-xs text-gray-500 mb-1.5 block">Ваша ставка</label>
                <input
                  type="number"
                  value={bidAmount}
                  onChange={e => setBidAmount(e.target.value)}
                  placeholder="$0"
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-500 transition mb-2"
                />
              ...
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>Продавец</span>
                <span className={`font-medium ${car.seller_trusted ? 'text-green-600' : 'text-gray-700'}`}>
