...
const mainMenuConfig: MenuItem[] = [
  ...
  {
    labelKey: "help.menu.supportConsultation",
    icon: "support",
    path: "/support",
  },
...
const secondLinksConfig = [
  "help.links.deliveryTimes",
  "help.links.about",
  "help.links.blog",
];
    ...
    >
      {name === "help" && (
        <>
          ...
          <path
            d="M9.8 9a2.4 2.4 0 0 1 4.6 1c0 1.8-2.4 2-2.4 3.6"
            {...common}
          />
      ...
      {name === "support" && (
        <>
          ...
          <path
            d="M5 13h3v5H6a1 1 0 0 1-1-1zM19 13h-3v5h2a1 1 0 0 0 1-1z"
            {...common}
          />
        </>
      )}
      ...
      {name === "diamond" && (
        <path d="M12 3 21 9l-9 12L3 9zM3 9h18" {...common} />
      )}
      {name === "lock" && (
        <>
      ...
      {name === "external" && (
        <path d="M8 8h8v8M16 8l-8 8M6 5h13v13" {...common} />
      )}
    </svg>
  );
}
  ...
  const helpSections = [
    {
      key: "purchase",
      title: t("help.title"),
      faqs: purchaseFaqsData,
    },
    ...helpSectionsConfig.map((section) => ({
      key: section.key,
      ...
      <div className="mx-auto grid w-[min(1160px,calc(100%-24px))] gap-10 md:w-[min(1160px,calc(100%-32px))] md:grid-cols-[280px_minmax(0,1fr)] md:gap-6">
        <aside className="space-y-4 md:space-y-6">
          <nav className="hidden overflow-hidden rounded border border-[#d9e0e7] bg-white md:block">
            {mainMenu.map((item, index) => {
              const className = `flex min-h-[60px] w-full items-center gap-3 border-b border-[#d9e0e7] px-4 text-left text-[13px] font-bold leading-4 text-[#0f2740] last:border-b-0 ${
                index === 0
                  ? "border-l-2 border-l-purple-600 bg-white"
                  : "hover:bg-slate-50" // Изменено на border-l-purple-600
              }`;
              const content = (
                <>
            ...
            <button
              ...
              className="block w-full px-4 pb-2 text-right text-[11px] font-medium text-purple-600" // Изменено на text-purple-600
            >
              {t("help.showAll")}
            </button>
          </nav>
          ...
          <nav className="overflow-hidden rounded border border-[#d9e0e7] bg-white">
            {secondLinks.map((link) => {
              const className =
                "flex min-h-[56px] w-full items-center justify-between border-b border-[#d9e0e7] px-4 text-left text-[13px] font-bold last:border-b-0 hover:bg-slate-50";
              const content = (
                <>
                  {link}
                  <span className="text-[#7e99b4]">
        ...
        <main className="min-w-0 space-y-9">
          {helpSections.map((section, sectionIndex) => (
            <section
              key={section.key}
              className={sectionIndex === 0 ? undefined : "md:hidden"}
            >
              <h2 className="mb-3 text-[19px] font-medium text-[#0f2740]">
                {section.title}
              </h2>
